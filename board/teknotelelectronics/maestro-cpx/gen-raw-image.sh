...
# size of 256MB
DEFAULT_ROOTFS_PARTITION_SIZE=524288
	...
	do
		...
		*)
		...
		rootfs1)
			...
			extrafs_param="-A $j:set:2 -u $j:${DEFAULT_SDCARD_PARTUUID}"
			;;
		*)
			extrafs_param=""
			;;
		...
		then
			...
			then
				image_size=$(du -Lb "${BINARIES_DIR}/$bin2flash" | tr '\t' ' ' | cut -d ' ' -f1)
		...
		then
			...
			then
				...
				then
					...
					exec_print "dd if=${BINARIES_DIR}/$bin2flash of=$FLASHLAYOUT_rawname conv=fdatasync,notrunc seek=1 bs=$offset"
					printf "\r[ FILLED ] part %d: %8s, image: %s \n" $j "$partName" "$bin2flash"
				else
					printf "\r[UNFILLED] part %d: %8s, image: %s (not present) \n" $j "$partName" "$bin2flash"
					echo "   [WARNING]: THE FILE ${BINARIES_DIR}/$bin2flash ARE NOT PRESENT."
					echo "   [WARNING]: THE PARTITION $partName ARE NOT FILL."
					WARNING_TEXT+="[WARNING]: THE PARTITION $partName ARE NOT FILL (file ${BINARIES_DIR}/$bin2flash are not present) #"
				fi
				j=$((j+1))
			else
...
then
	...
	FLASHLAYOUT_rawname="${BINARIES_DIR}/sdcard.raw"
	FLASHLAYOUT_infoname="${BINARIES_DIR}/how_to_update.txt"
	...
	debug "DUMP images dir path:       ${BINARIES_DIR}"
